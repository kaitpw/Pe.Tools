...
public static class JsonPresetComposer {
    public static JObject ExpandPresets(
        ...
        JObject ExpandObject(JObject obj) {
            if (obj.TryGetValue("$preset", out var presetToken)) {
                ...
                var expandedPreset = LoadExpandedPresetObject(resolvedPresetPath, originalPath);
        ...
        (string originalPath, SettingsPathing.ResolvedDirective resolvedDirective) ResolvePresetPath(JToken presetToken) {
    ...
    private static string ResolvePresetFilePath(SettingsPathing.ResolvedDirective directive) {
        var candidates = SettingsPathing.ResolveDirectiveFileCandidates(directive, allowToonFallback: false);
